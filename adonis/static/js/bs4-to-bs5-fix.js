/**
 * Safe admin enhancements for Jazzmin.
 * Keep this file minimal to avoid breaking native admin UI.
 */
document.addEventListener('DOMContentLoaded', function () {
    function addPasswordToggle(input) {
        ...
        btn.style.cssText = 'line-height:1;padding:6px 10px;';
    ...
    // Password toggle in user forms.
    document.querySelectorAll('input[type="password"]').forEach(addPasswordToggle);

    // Explicit logout button in top navbar.
    setTimeout(function () {
        var nav =
            document.querySelector('.main-header .navbar-nav.ml-auto') ||
            document.querySelector('.main-header .navbar-nav:last-child');

        if (!nav || document.getElementById('explicit-logout-btn')) return;

        var item = document.createElement('li');
        item.className = 'nav-item';
        item.innerHTML =
            '<a href="/admin/logout/" id="explicit-logout-btn" class="nav-link" ' +
            'style="color:#dc2626 !important;font-weight:600;">' +
